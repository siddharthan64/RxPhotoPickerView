...
class AssetGridViewController: UICollectionViewController {
    ...
    var reEntryModelIndexes: [SelectionModel] = []

    fileprivate var fetchResult: PHFetchResult<PHAsset>!
    fileprivate let imageManager = PHCachingImageManager()
    fileprivate var thumbnailSize: CGSize!
    fileprivate var previousPreheatRect = CGRect.zero
    fileprivate var modelPublisher: PublishSubject<[SelectionModel]> = PublishSubject<[SelectionModel]>()

    var modelObservable: Observable<[SelectionModel]> {
        return modelPublisher.asObservable()
    }
    ...
    @objc private func dismissView(_: Any) {
        let filteredModels = modelIndexes.filter { $0.isSelected == true }
        modelPublisher.onNext(filteredModels)
